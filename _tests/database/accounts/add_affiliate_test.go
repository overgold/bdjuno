...
func TestRepository_SaveAddAffiliate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := r.SaveAddAffiliate(tt.args.msg...); (err != nil) != tt.wantErr {
...
func TestRepository_GetAddAffiliate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "valid",
			args: args{
				accfilter: filter.NewFilter().SetArgument(types.FieldCreator, "vcg1ljs7p2p9ae3en8knr3d3ke8srsfcj2zjvefv1g"),
			},
			want: []*accountstypes.MsgAddAffiliate{
				{
					Creator:         "vcg1ljs7p2p9ae3en8knr3d3ke8srsfcj2zjvefv1g",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := r.GetAddAffiliate(tt.args.accfilter)
