...
// UpdateGenesisState - method that updates in a database (overgold_feeexcluder_genesis_state).
func (r Repository) UpdateGenesisState(genesisStateID uint64, gs fe.GenesisState) error {
	tx, err := r.db.BeginTxx(context.Background(), &sql.TxOptions{})
	if err != nil {
		return err
	}
	...
	// 1) update genesis state
	q := `UPDATE overgold_feeexcluder_genesis_state SET
				 address_count = $1,
				 daily_stats_count = $2
			 WHERE id = $3`

	m := toGenesisStateDatabase(genesisStateID, gs)
	if _, err = tx.Exec(q, m.AddressCount, m.DailyStatsCount); err != nil {
		return errs.Internal{Cause: err.Error()}
	}

	// 2) update address
	for _, a := range gs.AddressList {
		if err = r.UpdateAddress(tx, a); err != nil {
			return err
		}
	}

	// 3) update daily stats and get ids
	stats, err := r.GetAllM2MGenesisStateDailyStats(filter.NewFilter().
		SetArgument(types.FieldGenesisStateID, genesisStateID))
	if err != nil {
		return err
	}
	...
	// 4) update stats
	for _, s := range gs.StatsList {
		if err = r.UpdateStats(tx, s); err != nil {
			return err
		}
	}

	// 5) update tariffs and get ids
	tariffs, err := r.GetAllM2MGenesisStateTariffs(filter.NewFilter().
		SetArgument(types.FieldGenesisStateID, genesisStateID))
	if err != nil {
		return err
	}
